...
const DATA_DIR = path.join(__dirname, '../../data/word-counts');
...
function generateStatusHtml() {
  ...
  return `
    ...
    <html>
    <head>
      <title>Word Count Progress</title>
...
async function runWordCount(topN = 5) {
  try {
    await init();
    ...
    if (!progress.agencies) {
      ...
      // Sort agencies by size (number of CFR references)
      const agencies = response.data.agencies.sort((a, b) => {
        const aRefs = a.cfr_references?.length || 0;
        const bRefs = b.cfr_references?.length || 0;
        return bRefs - aRefs;
      });
      
      // Take only top N agencies
    ...
    console.log(`Word count complete! Results saved to data/word-counts/word_count_report.json`);
...
module.exports = { runWordCount };

// Direct execution
if (require.main === module) {
  // Get number of agencies from command line, default to 5
  const topN = parseInt(process.argv[2]) || 5;
  console.log(`Starting word count for top ${topN} agencies...`);
  runWordCount(topN).catch(console.error);
} 